...
void do_send(osjob_t* j){
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        sprintf(&floatStr[0], "  T = ");
        dtostrf(temperature,6,1,&floatStr[strlen(floatStr)]);
        sprintf(&floatStr[strlen(floatStr)], " %cC", 0x27);
        display.drawString(0, 3, floatStr);

        sprintf(&floatStr[0], "  P = ");
        dtostrf(pressure,4,0,&floatStr[strlen(floatStr)]);
        sprintf(&floatStr[strlen(floatStr)], "   hPa");
        display.drawString(0, 5, floatStr);

        sprintf(&floatStr[0], " rH = ");
        dtostrf(humidity,6,1,&floatStr[strlen(floatStr)]);
        sprintf(&floatStr[strlen(floatStr)], " %%");
        display.drawString(0, 7, floatStr);

        int batt = (int)(vbat * 10); 
...
void setup() {
  ...
  if(bme280.begin(I2C_BME280_ADDR)){
    display.drawString(0,0,"BME280 Example");
  } else {
